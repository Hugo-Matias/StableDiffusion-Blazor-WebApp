...
/// <summary>
/// Qwen Txt2Img workflow implementation using the fluent builder API.
/// Uses LoadDiffusionWithPrompts (qwen_image CLIP) + ModelSamplingAuraFlow + ClownsharKSampler.
/// Pipeline: LoadDiffusionWithPrompts -> EmptyLatent(SD3) -> ModelSamplingAuraFlow -> Sample -> VaeDecode -> [SeedVR2] -> [Detailer] -> Save
/// </summary>
public class QwenTxt2ImgWorkflow : IWorkflowBuilder
{
    // Core fragments
    ...
    public WorkflowMetadata Metadata => new()
    {
        ...
        [
            new WorkflowAsset
            {
                ...
                DefaultValue = "qwen_image_fp8_e4m3fn.safetensors",
                Order = 1,
                ColumnSize = 4
            },
            new WorkflowAsset
    ...
    public ComfyWorkflow Build(GenerationParameters parameters)
    {
        ...
        // 1. Load Qwen models (UNet + CLIP qwen_image + VAE) with prompts
        _loadDiffusionWithPromptsFragment.Build(builder, registry, new LoadDiffusionWithPromptsFragment.Parameters
        {
            UnetName = parameters.Assets?.GetValueOrDefault("Model") ?? "qwen_image_fp8_e4m3fn.safetensors",
        ...
        // 2. Create empty latent (Qwen uses EmptySD3LatentImage)
        ...
        // 3. Apply ModelSamplingAuraFlow shift
        _modelSamplingAuraFlowFragment.Build(builder, registry, new ModelSamplingAuraFlowFragment.Parameters
        {
            ModelShift = samplerFragment?.GetDouble("model_shift", 3.10) ?? 3.10
        });
        ...
        if (seedVr2Fragment?.IsActive == true)
        {
            _seedVR2UpscaleFragment.Build(builder, registry, new SeedVR2UpscaleFragment.Parameters
            {
                ...
                Seed = seedVr2Fragment.GetLong("seedvr2_seed", resolvedSeed),
