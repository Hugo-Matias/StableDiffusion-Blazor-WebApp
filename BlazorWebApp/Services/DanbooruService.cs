...
namespace BlazorWebApp.Services
{
    public class DanbooruService
    {
        ...
        public DanbooruService(HttpClient httpClient, IOptions<DanbooruOptions> danbooruOptions)
        {
            _httpClient = httpClient;
            _httpClient.BaseAddress = new Uri("https://hijiribe.donmai.us");
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.UTF8.GetBytes($"{danbooruOptions.Value.Login}:{danbooruOptions.Value.ApiKey}")));
