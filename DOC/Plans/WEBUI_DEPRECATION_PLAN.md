...
|------|-------|-------------|
| 2025-01-14 | Phase 6.5 | ?? **Part 4 Complete (75%)!** Part 1: Removed 13 script form files. Part 2: Removed SDAPIService from 5 services. Part 3: Removed IParameterFactory interface, cleaned 9 files, fixed 3 components. **Part 4: Removed Scripts property + script parameter classes from Txt2ImgParameters and Img2ImgParameters models**. **Removed** CreateScriptParameters and ParseDetailerModelLoras methods from Parser.cs. **Remaining:** Fix Parser syntax error, remove script settings from AppSettings (~800 lines), remove ControlNetPreprocessor enum, remove WebUI mappers from ParameterMapper, remove UltimateUpscaleForm. **Still ~7 compilation errors** - will fix in Parts 5-11. Progress: 75% of Phase 6.5 complete. |
