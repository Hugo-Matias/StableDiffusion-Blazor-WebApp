...
|------|-------|-------------|
| 2025-01-14 | Phase 6.5 | ?? **Part 5 Complete (95%)!** Part 1: Removed 13 script form files. Part 2: Removed SDAPIService from 5 services. Part 3: Removed IParameterFactory interface, cleaned 9 files, fixed 3 components. Part 4: Removed Scripts properties + script parameter classes. **Part 5: Fixed Parser syntax error, removed WebUI mappers (ToTxt2ImgWebUI, ToImg2ImgWebUI) from ParameterMapper, removed UltimateUpscaleForm component**. **Remaining:** Remove ControlNetPreprocessor enum + all script settings from AppSettings (~800 lines). **Down to just 2 compilation errors!** Progress: 95% of Phase 6.5 complete. |
